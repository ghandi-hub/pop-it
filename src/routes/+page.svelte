...
<svelte:head>
  ...
  <meta
    name="description"
    content="Fast-paced tactile brutalist Pop It web game. Press all bubbles before the timer expires!"
  />
</svelte:head>

<div
  class="min-h-screen w-full bg-[#F4EBD0] py-2 px-1 sm:py-4 sm:px-4 flex flex-col items-center justify-between text-[#111111]"
>
  <!-- Top Navigation / Title Header -->
  <header
    class="w-full max-w-xl flex items-center justify-between py-2 px-1 select-none"
  >
    <div class="flex items-center gap-2">
      <span class="w-4 h-4 rounded-full bg-[#FF5C35] border-2 border-[#111111]"
      ></span>
      <span class="font-display font-black text-lg tracking-tight uppercase"
        >POP IT TIME ATTACK</span
      >
    </div>
    <div class="flex items-center gap-2">
      <a
  ...
  <main
    class="w-full max-w-xl flex-1 flex flex-col items-center justify-center my-2"
  >
    <DeviceMockup>
      <!-- IDLE SCREEN -->
  ...
  <footer
    class="w-full max-w-xl text-center py-2 text-xs font-bold text-[#111111]/60 uppercase tracking-wider select-none"
  >
    Press all bubbles before the timer expires • Rapid pops build combo
    multiplier
  </footer>
