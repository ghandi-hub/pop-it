...
export class PopItGame {
   ...
   * Resets combo streak to 0 and deducts 0.1s penalty.
   */
  ...
  public popBubble(id: number): boolean {
    ...
    if (
      bubble.id === this.lastPoppedBubbleId &&
      now - this.lastPopTimestamp < 100
    ) {
      return false;
    }
    ...
    if (bubble.type === "golden") {
      ...
      const newRemainingMs = Math.min(
        MAX_TIMER_CAP * 1000,
        currentRemainingMs + 1200,
      );
  ...
  private handleLevelClear(): void {
    ...
    // Award bonus score
    const levelClearBonus = calculateLevelClearScore(
      this.level,
      currentRemaining,
    );
    ...
    const tick = (now: number) => {
      ...
      if (this.goldenBubbleId !== null) {
        ...
        if (
          goldenBubble &&
          goldenBubble.active &&
          !goldenBubble.pressed &&
          goldenBubble.type === "golden"
        ) {
          ...
          if (goldenRemainingMs <= 0) {
            ...
            const secondsLeft = Math.max(
              0.1,
              Math.round((goldenRemainingMs / 1000) * 10) / 10,
            );
